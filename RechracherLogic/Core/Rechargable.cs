...
{
    [Serializable]
    public class Rechargable : IRechargable<float>
    {
        [FoldoutGroup(EditorConstants.debugGroup, 100)]
        [SerializeField]
        DeltaRange charge = new DeltaRange();
        [FoldoutGroup(EditorConstants.debugGroup)]
        [SerializeField]
        Delta rechargeSpeed = new Delta(-1);
        ...
        public Rechargable(float value = 100, float minBorder = 0, float maxBorder = 100, float rechargeSpeed = -1f,
            TimeMode timeMode = TimeMode.scaledTime, float updateDelay = ComplexFieldValues.updateRate)
        {
            charge = new DeltaRange(value, minBorder, maxBorder);
        ...
        public void StartRecharge()
        {
            ...
            disposable = Observable.Interval(TimeSpan.FromSeconds(updateDelay)).Subscribe((_) => RechargeRoutine());
        }
        ...
        public static implicit operator float(Rechargable deltaValue)
        {
